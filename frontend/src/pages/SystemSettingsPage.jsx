...
function SystemSettingsPage() {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      handleApiError(err, "Unable to fetch system settings");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApiError = (err, defaultMessage) => {
    ...
    if (err.response) {
      ...
      if (err.response.status === 401) {
        ...
        setError(`${defaultMessage}. Error code: ${err.response.status}`);
      }
    } else if (err.request) {
      // Không nhận được phản hồi từ server
      setError(`${defaultMessage}. Unable to connect to server.`);
    } else {
      // Lỗi khác
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      handleApiError(err, "Unable to fetch document list");
      setLoading(false);
    }
  };
  ...
  const fetchDocumentChunks = async (documentId) => {
    ...
    try {
      ...
      handleApiError(err, "Unable to fetch chunk list");
      setLoading(false);
    }
  };
  ...
  const fetchChunkContent = async (documentId, chunkIndex) => {
    ...
    try {
      ...
      handleApiError(err, "Unable to fetch chunk content");
      setLoading(false);
    }
  };
  ...
  const handleUpload = async (e) => {
    ...
    if (!file || !documentTitle) {
      setError("Please select a file and enter document name");
      return;
    }
    ...
    try {
      ...
      handleApiError(err, "Unable to upload document");
      setIsUploading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      handleApiError(err, "Unable to save system settings");
    } finally {
      setLoading(false);
    }
  };
  ...
  const checkEmbeddingStatus = async (documentId) => {
    ...
    try {
      ...
      handleApiError(err, "Unable to fetch embedding status");
      return null;
    }
  };
  ...
  const embedDocument = async () => {
    ...
    try {
      ...
      showSuccess(`Embedded ${response.data.chunks_count} chunks successfully`);
    } catch (err) {
      setIsEmbedding(false);
      handleApiError(err, "Unable to create embedding for document");
    }
  };
  ...
  const saveDocumentEdit = async () => {
    ...
    try {
      ...
      showSuccess("Document name updated successfully");
    } catch (err) {
      handleApiError(err, "Unable to update document name");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteDocument = async () => {
    ...
    try {
      ...
      handleApiError(err, "Unable to delete document");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveChunkEdit = async () => {
    ...
    try {
      ...
      showSuccess("Chunk updated successfully");
    } catch (err) {
      handleApiError(err, "Unable to update chunk");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteChunk = async () => {
    ...
    try {
      ...
      showSuccess("Chunk deleted successfully");
    } catch (err) {
      handleApiError(err, "Unable to delete chunk");
    } finally {
      setLoading(false);
    }
  };
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Test Settings</h2>
              
              <div className="mb-4">
                <label className="block text-gray-700 mb-2">Maximum questions per test</label>
                <input
                  type="number"
            ...
            {/* Cài đặt hệ thống */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">System Settings</h2>
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            <div className="md:col-span-4 bg-gray-50 p-4 rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Uploaded Documents</h2>
            ...
            <div className="md:col-span-8">
              {selectedDocument ? (
                <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            Uploaded: {selectedDocument.upload_date} | {selectedDocument.chunks_count} chunks
                          </p>
                          <p className="text-sm text-gray-500 mt-1">
                            Total words: {selectedDocument.total_words} | File type: {selectedDocument.file_type}
                          </p>
                            ...
                            <div className="mt-2">
                              <p className={`text-sm ${embeddingStatus.is_fully_embedded ? 'text-green-600' : 'text-amber-600'}`}>
                                ...
                                {embeddingStatus.is_fully_embedded 
                                  ? `Embedded ${embeddingStatus.embedded_chunks}/${embeddingStatus.total_chunks} chunks` 
                                  : `Embedded ${embeddingStatus.embedded_chunks}/${embeddingStatus.total_chunks} chunks`}
                              </p>
                            </div>
                          )}
                        ...
                        <div className="flex space-x-2">
                          ...
                          <button 
                            ...
                            title="Create vector embedding for chunks"
                          >
                            {isEmbedding 
                              ? 'Embedding...' 
                              : embeddingStatus && embeddingStatus.is_fully_embedded
                  ...
                  {showDeleteDocumentConfirm && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                      <div className="bg-white p-6 rounded-lg max-w-md w-full">
                        ...
                        <p>Are you sure you want to delete document <span className="font-semibold">{selectedDocument.title}</span>?</p>
                        <p className="text-red-600 text-sm mt-2">Note: This action cannot be undone and will delete all chunks belonging to the document.</p>
                  ...
                  {/* Danh sách chunks */}
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    <h3 className="text-lg font-semibold mb-3">Chunk List</h3>
                        ...
                        {documentChunks.map(chunk => (
                          ...
                          >
                            ...
                            <div className="text-xs text-gray-500">
                              Index: {chunk.index} | {chunk.word_count} words {chunk.tokens && `| ${chunk.tokens} tokens`}
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      {isEditingChunk ? (
                        ...
                        <div>
                          ...
                          <div className="mb-2 text-sm text-gray-600">
                            <span className="mr-3">{selectedChunk.word_count} words</span>
                  ...
                  {showDeleteChunkConfirm && selectedChunk && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                      <div className="bg-white p-6 rounded-lg max-w-md w-full">
                        <h3 className="text-lg font-bold mb-4">Confirm deletion</h3>
                        <p>Are you sure you want to delete chunk <span className="font-semibold">#{selectedChunk.index}</span>?</p>
                        <p className="text-red-600 text-sm mt-2">Note: This action cannot be undone.</p>
                        <div className="flex justify-end space-x-3 mt-6">
                          <button 
      ...
      case "upload":
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Form tải tài liệu */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Upload New Document</h2>
              ...
              <form onSubmit={handleUpload}>
                ...
                <div className="mb-4">
                  <label className="block text-gray-700 mb-2">Document File (PDF, DOCX, TXT)</label>
                  <input
                    type="file"
                ...
                >
                  {isUploading ? 'Uploading...' : 'Upload and Chunk'}
                </button>
              </form>
            </div>
            ...
            {/* Cài đặt chunk */}
            <div className="bg-gray-50 p-4 rounded-lg">
              <h2 className="text-lg font-semibold mb-4 text-gray-700 border-b pb-2">Chunk Settings</h2>
              
              <div className="mb-4">
                <label className="block text-gray-700 mb-2">Chunk Size (tokens)</label>
                <input
                  type="number"
                ...
                />
                <p className="text-sm text-gray-500 mt-1">
                  Maximum token count per chunk. Recommended value: 1000 tokens (~750 words)
                </p>
              </div>
              
              <div className="mb-4">
                <label className="block text-gray-700 mb-2">Overlap (tokens)</label>
                <input
                  type="number"
                ...
                />
                <p className="text-sm text-gray-500 mt-1">
                  Overlapping tokens between consecutive chunks. Recommended value: 100-200 tokens
                </p>
              </div>
              
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg mt-6">
                <h3 className="font-medium text-blue-800 mb-2">Notes about chunking</h3>
                <ul className="list-disc list-inside text-sm text-blue-700 space-y-1">
                  <li>Chunk size is measured in tokens (processing units for LLM models)</li>
                  <li>Smaller chunk sizes help with more accurate answers but increase the number of chunks</li>
                  <li>Overlapping helps maintain context between consecutive chunks</li>
                  <li>Adjust settings based on document characteristics and the model being used</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <p className="text-gray-600 mb-4">Upload and manage documents and images for the system</p>
              <button 
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <p className="text-gray-600 mb-4">Adjust system settings such as test time limits, number of questions</p>
              <button 
                className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
                onClick={() => alert('Feature under development')}
              >
